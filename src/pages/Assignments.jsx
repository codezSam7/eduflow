        ...
        return (
            <div className="space-y-8">
                <div className="flex items-center justify-between">
                    ...
                    <button onClick={() => setActiveTab('list')} className="text-indigo-600 hover:text-indigo-800 font-medium">
                        ← Back to list
                    </button>
                </div>
                <form className="bg-white rounded-xl shadow p-8 space-y-6">
                    <div>
                        ...
                        <input className="w-full px-4 py-3 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-indigo-500" placeholder="e.g. Quadratic Equations Homework" />
                    </div>
                    <div>
                        <label className="block text-sm font-medium text-gray-700 mb-1">Class / Subject</label>
                        <select className="w-full px-4 py-3 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-indigo-500 bg-white">
                            <option>JSS 3A - Mathematics</option>
                            <option>JSS 2B - English</option>
                            <option>SSS 1C - Basic Science</option>
                        </select>
                    </div>
                    <div>
                        <label className="block text-sm font-medium text-gray-700 mb-1">Due Date</label>
                        <input type="date" className="w-full px-4 py-3 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-indigo-500" />
                    </div>
                    <div>
                        <label className="block text-sm font-medium text-gray-700 mb-1">Instructions / Description</label>
                        <textarea rows={6} className="w-full px-4 py-3 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-indigo-500" placeholder="Detailed instructions..." />
                    </div>
                    <button type="button" className="w-full md:w-auto px-8 py-3 bg-indigo-600 text-white font-medium rounded-lg hover:bg-indigo-700">
                        Create Assignment
                    </button>
                </form>
            </div>
                            ...
                            <tbody className="divide-y divide-gray-200">
                                {submissions.map((sub) => (
                                    <tr key={sub.studentId} className="hover:bg-gray-50">
                                        ...
                                        <td className="px-6 py-4">
                                            {sub.submitted ? (
                                                <input type="number" min="0" max={sub.maxGrade} defaultValue={sub.grade} className="w-20 px-2 py-1 border border-gray-300 rounded text-center" />
                                            ) : '-'}
                                        </td>
                                        <td className="px-6 py-4">
                                            {sub.submitted && (
                                                <input type="text" defaultValue={sub.feedback} className="w-full px-3 py-1 border border-gray-300 rounded" placeholder="Enter feedback..." />
                                            )}
                                        </td>
                                    </tr>
                                ))}
    ...
    return (
        <div className="space-y-8">
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                ...
                <button onClick={() => setActiveTab('create')} className="flex items-center gap-2 px-6 py-3 bg-indigo-600 text-white font-medium rounded-lg hover:bg-indigo-700">
                    <PlusCircle size={20} />
                        ...
                        <tbody className="divide-y divide-gray-200">
                            {filteredAssignments.map((assignment) => (
                                <tr key={assignment.id} className="hover:bg-gray-50">
                                    ...
                                    <td className="px-6 py-4 text-sm text-gray-600">{assignment.className}</td>
                                    <td className="px-6 py-4 text-sm text-gray-600">{assignment.dueDate}</td>
                                    <td className="px-6 py-4 text-sm">{assignment.submissions} submitted • {assignment.graded} graded</td>
